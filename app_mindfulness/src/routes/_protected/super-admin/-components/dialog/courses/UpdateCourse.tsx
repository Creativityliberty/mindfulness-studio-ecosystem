...
import { useUpdateCourse } from "hooks/super-admin";
...
export function UpdateCourse({
  ...
  const resetMutation = updateCourse.reset;
  React.useEffect(() => {
    if (course && open) {
      form.reset({
      ...
      resetMutation();
      setImagePreview(course.image ?? undefined);
      setImageFile(undefined);
    }
  }, [course, open, form, resetMutation]);
  ...
  React.useEffect(() => {
    const error = updateCourse.error;
    if (!error) return;
  ...
  const onSubmit = async (data: CourseFormData) => {
    ...
    try {
      ...
      onOpenChange(false);
      myGoeyToast("success", "Formation mise à jour", {
        description: "Les modifications ont été enregistrées avec succès.",
      });
    } catch {
      // Géré par useEffect sur updateCourse.error
    }
  };
        ...
        <form onSubmit={form.handleSubmit(onSubmit)}>
          <div className="space-y-4 py-4">
            ...
            <Controller
              ...
              render={({ field, fieldState }) => (
                <Field data-invalid={fieldState.invalid}>
                  <FieldLabel htmlFor="update-course-name">Nom</FieldLabel>
                  <Input
                    {...field}
            ...
            <Controller
              ...
              render={({ field, fieldState }) => (
                <Field data-invalid={fieldState.invalid}>
                  <FieldLabel htmlFor="update-course-description">
                    Description
                  </FieldLabel>
                  <Textarea
                    {...field}
            ...
            <div className="grid grid-cols-2 gap-4">
              <Controller
                ...
                render={({ field, fieldState }) => (
                  <Field data-invalid={fieldState.invalid}>
                    <FieldLabel htmlFor="update-course-price">
                      Prix (€)
                    </FieldLabel>
                    <Input
                      {...field}
                      id="update-course-price"
                      type="number"
                      min={0}
                      step="0.01"
                      aria-invalid={fieldState.invalid}
              ...
              <Controller
                ...
                render={({ field, fieldState }) => (
                  <Field data-invalid={fieldState.invalid}>
                    <FieldLabel htmlFor="update-course-duration">
                      Durée (semaines)
                    </FieldLabel>
                    <Input
                      {...field}
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={updateCourse.isPending}>
              {updateCourse.isPending ? "Mise à jour..." : "Mettre à jour"}
            </Button>
          </DialogFooter>
        </form>
      </DialogPopup>
