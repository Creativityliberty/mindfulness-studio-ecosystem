...
class CourseController extends Controller
{
    ...
    {
        return CourseResource::collection($this->courseService->getAll($request->only([
            'category_id',
            'is_published',
        ])));
    }
    ...
    {
        try {
            return new CourseResource($this->courseService->save($request->validated(), $request->file('image')));
    ...
    {
        try {
            return new CourseResource($this->courseService->update($request->validated(), $id, $request->file('image')));
        } catch (\Exception $exception) {
            report($exception);
